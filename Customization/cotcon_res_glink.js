...
function printFooterBlock()
{
	...
	write('<input id="Ok" name="Ok" type="button" class="button" value="' + jspResArr.get("FLT031311") + '"	onClick="javascript:return cotcon_res_ONCLICK1(this);"" hotKeyId="Go">');
	writeFooter();
	write('</div>');
	}
}//End function
...
function cotcon_res_ONCLICK1(obj)
{
	var retVal = "";
	if (pre_ONCLICK('cotcon_res',obj) == false) { 
		return false;
	}
	if ((retVal = fnLastButtonClick()) == false) {
		return false;
	}
	if (post_ONCLICK('cotcon_res',obj) == false) { 
		return false;
	}
